...
// Check the sidebar state from localStorage
const savedState = localStorage.getItem('sidebarState') || 'expanded' // Default to 'expanded' if no state is saved

// Set initial state based on saved state
if (savedState === 'collapsed') {
  setOpenMobile(false) // Collapse if saved state is 'collapsed'
} else {
  setOpenMobile(true) // Expanded by default
}

// Watch for state changes and save the state to localStorage
watch(state, (newState) => {
  console.log('Sidebar state changed:', newState)
  localStorage.setItem('sidebarState', newState) // Save to localStorage
})
</script>
